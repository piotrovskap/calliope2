...
Every box or card heading must use the same title treatment as `General info`:

- Satoshi, 12 px, weight 700,
- uppercase text,
- secondary/muted text color (`#717171`),
- `0.02em` letter spacing,
- 16 px spacing between the title and the box content.

This treatment applies consistently to section titles such as `General info`, `Address info`, `Consent`, `Activity log`, `Vehicles` and `Deals & transactions`. A box title must not inherit a larger generic heading style from its semantic `h2` or `h3` element.
