...
// .wrangler/tmp/bundle-RhLrJN/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-RhLrJN/middleware-insertion-facade.js
init_modules_watch_stub();
...
var MAX_AGENT_TURNS = 8;
var MAX_DOC_CHARS = 12e4;
var TOOL_CHAR_BUDGET = 45e4;
var ALLOWED_NAV_URLS = /* @__PURE__ */ new Set([
  "/",
  "/specs/",
  "/tracker/",
  "/docs/",
...
var TOOLS = [
  {
    ...
    description: "Search the knowledge-base index (95+ documents: memory, wiki, docs, specs, live trackers). Returns matching documents with path, title, and summary. Call this first for any project question, then read_doc the best matches.",
    input_schema: {
      type: "object",
      properties: {
        query: {
  ...
  {
    name: "read_doc",
    description: "Read a file from the knowledge base by its path (as returned by search_knowledge). Returns the full text (truncated if very large). Markdown, JSON, text, and code files (.py/.js/.html/.toml) supported.",
    input_schema: {
      type: "object",
      properties: {
        path: {
  ...
  {
    name: "search_graph",
    description: "Search the project knowledge graph (entities: people, systems, databases, data sources, decisions, concepts \u2014 with typed relationships between them). Returns matching entities and all their relationships with neighbor names. Use for who/what/how-related questions, e.g. 'Ron Mulder', 'DWRPT', 'Acceptor'.",
    input_schema: {
      type: "object",
      properties: {
        query: {
  ...
  {
    name: "navigate",
    description: "Offer the user a portal page as a clickable navigation chip in the chat UI. Use for pages that answer or illustrate their question. The user decides whether to click.",
    input_schema: {
      type: "object",
      properties: {
        url: {
...
function searchGraph(graph, query) {
  ...
  for (const { n } of hits) {
    const id = n.id;
    const rels = graph.edges.filter((e) => e.source === id || e.target === id).slice(0, 30).map(
      (e) => e.source === id ? `  -> ${e.type} -> ${e.target}` : `  <- ${e.type} <- ${e.source}`
    );
    out.push(
...
async function loadBrain(env) {
  if (!brainCache) {
    try {
      ...
      brainCache = { nodes: [], edges: [], byId: /* @__PURE__ */ new Map() };
    }
  }
  return brainCache;
}
__name(loadBrain, "loadBrain");
function searchBrain(brain, query) {
  const q = query.toLowerCase();
  const terms = q.split(/[^a-z0-9]+/).filter((t) => t.length > 1);
  const hits = brain.nodes.map((n) => {
    ...
    return { n, score };
  }).filter((h) => h.score > 0).sort((a, b) => b.score - a.score).slice(0, 6);
  if (!hits.length) return "No matching brain records. Try a different name or keyword.";
  const labelOf = /* @__PURE__ */ __name((id) => brain.byId.get(id)?.label || id, "labelOf");
  const out = [];
  for (const { n } of hits) {
    const id = n.id;
    ...
    const rels = brain.edges.filter((e) => e.src === id || e.dst === id).slice(0, 25).map(
      (e) => e.src === id ? `  -> ${e.type} -> ${labelOf(e.dst)}` : `  <- ${e.type} <- ${labelOf(e.src)}`
    );
    out.push(
      `${n.label || id} (${meta.join(" \xB7 ")})${n.source?.path ? ` [source: ${n.source.path}]` : ""}
` + (rels.length ? rels.join("\n") : "  (no relationships recorded)")
    );
  }
  return out.join("\n\n");
}
__name(searchBrain, "searchBrain");
async function readDoc(env, path2) {
  const clean = cleanDocPath(path2);
...
function cleanDocPath(path2) {
  const raw = String(path2 || "").replace(/\\/g, "/").replace(/^\/+/, "");
...
__name(cleanDocPath, "cleanDocPath");
function isAllowedNavUrl(url) {
  return ALLOWED_NAV_URLS.has(String(url || ""));
}
__name(isAllowedNavUrl, "isAllowedNavUrl");
async function handleChat(request, env) {
  ...
  const messages = history.map((m) => ({
    ...
    content: String(m.content).slice(0, 32e3)
  }));
  const actions = [];
  const textsSeen = [];
  const sourcesRead = [];
    ...
    for (const block of response.content) {
      ...
      if (typeof result === "string") {
        ...
        if (result.length > remaining) {
          result = remaining > 400 ? result.slice(0, remaining) + "\n\n[context budget reached \u2014 do not read more documents; answer from what you have already read]" : "[context budget reached \u2014 do not call more tools; answer now from the material already read]";
        }
  ...
  const deLink = /* @__PURE__ */ __name((t) => t.replace(/\[([^\]]+)\]\(([^)]*)\)/g, "$1").trim(), "deLink");
  let reply = deLink(
  ...
  return json({
    reply: reply || (degraded ? "I hit a limit pulling that together \u2014 try a narrower question, or switch to a lighter answer mode (terse/brief)." : "I could not produce an answer \u2014 try rephrasing."),
    actions,
    sources
  });
}
